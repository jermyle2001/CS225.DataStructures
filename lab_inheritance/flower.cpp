...
Flower::Flower(const Vector2& center)
{
    ...
    /* RECTANGLE */ stem = new Rectangle(stem_center, STEM_COLOR, STEM_WIDTH, STEM_HEIGHT);

    /* CIRCLE */ pistil = new Circle(Vector2(center.x(), center.y() - PISTIL_RADIUS / 2),
                     PISTIL_COLOR, PISTIL_RADIUS);


    const Vector2 leaf_center(stem_center.x() - STEM_WIDTH / 2 + 1,
                              stem_center.y() + STEM_HEIGHT / 4);
    /* TRIANGLE */ leaf = new Triangle(LEAF_COLOR,
